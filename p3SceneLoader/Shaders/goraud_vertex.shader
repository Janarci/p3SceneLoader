...
void main()
{
	...
	vec3 modelViewNormal = vec3(u_model * vec4(a_normal, 0.0));
	vec3 vertexPos = vec3(u_model * vec4(a_pos, 1.0));
	vec3 lightVector = normalize(u_light_pos - vertexPos);
	
	vec3 diffuse = vec3(max(dot(modelViewNormal, lightVector), 0.0));

	vertexColor = u_ambient_color+ diffuse;
	...
	*/
	//vec3 diffuse = vec3(max(dot(a_normal, lightVector), 0.0));
	//vec3 diffuse = vec3(max(dot(modelViewNormal, lightVector), 0.0));
